...
function(myproj_version_to_int VersionString VersionIntVar)
...
# main target.
function(add_myproj_target TARGET_NAME ADD_TO_ALL)
  if (ADD_TO_ALL)
...
# Add myproj examples
function(add_myproj_examples EXE_SRCS)
  set(EXE_PREFIX "")
  ...
  foreach(SRC_FILE IN LISTS ${EXE_SRCS})
    ...
    add_dependencies(${MYPROJ_ALL_EXAMPLES_TARGET_NAME} ${EXE_NAME})
    if (EXE_NEEDED_BY)
      add_dependencies(${EXE_NEEDED_BY} ${EXE_NAME})
    endif()
    add_dependencies(${EXE_NAME}
      ${MYPROJ_EXEC_PREREQUISITES_TARGET_NAME} ${EXE_PREREQUISITE})

    target_link_libraries(${EXE_NAME} myproj)
    if (MYPROJ_USE_MPI)
      # Not needed: (myproj already links with MPI_CXX_LIBRARIES)
...
# A slightly more versatile function for adding miniapps to MYPROJ
function(add_myproj_miniapp MYPROJ_EXE_NAME)
  ...
  add_executable(${MYPROJ_EXE_NAME} ${MAIN_LIST}
    ${EXTRA_SOURCES_LIST} ${EXTRA_HEADERS_LIST})
  add_dependencies(${MYPROJ_ALL_MINIAPPS_TARGET_NAME} ${MYPROJ_EXE_NAME})
  add_dependencies(${MYPROJ_EXE_NAME} ${MYPROJ_EXEC_PREREQUISITES_TARGET_NAME})

  # Append the additional libraries and options
  if (LIBRARIES_LIST)
    if(CMAKE_VERSION VERSION_GREATER 2.8.11)
      target_link_libraries(${MYPROJ_EXE_NAME} PRIVATE ${LIBRARIES_LIST})
    else()
      target_link_libraries(${MYPROJ_EXE_NAME} ${LIBRARIES_LIST})
    endif()
  endif()
  if (EXTRA_OPTIONS_LIST)
    string(REPLACE ";" " " EXTRA_OPTIONS_STRING "${EXTRA_OPTIONS_LIST}")
    message(STATUS "${MYPROJ_EXE_NAME}: add flags \"${EXTRA_OPTIONS_STRING}\"")
    if(CMAKE_VERSION VERSION_GREATER 2.8.11)
      target_compile_options(${MYPROJ_EXE_NAME} PRIVATE ${EXTRA_OPTIONS_LIST})
    else()
      get_target_property(THIS_COMPILE_FLAGS ${MYPROJ_EXE_NAME} COMPILE_FLAGS)
      ...
      set_target_properties(${MYPROJ_EXE_NAME}
        PROPERTIES COMPILE_FLAGS ${THIS_COMPILE_FLAGS})
    endif()
  endif()
  if (EXTRA_DEFINES_LIST)
    target_compile_definitions(${MYPROJ_EXE_NAME} PRIVATE ${EXTRA_DEFINES_LIST})
  endif()

  # Handle the MPI separately
  if (MYPROJ_USE_MPI)
    # Add MPI_CXX_LIBRARIES, in case this target does not link with myproj.
    if(CMAKE_VERSION VERSION_GREATER 2.8.11)
      target_link_libraries(${MYPROJ_EXE_NAME} PRIVATE ${MPI_CXX_LIBRARIES})
    else()
      target_link_libraries(${MYPROJ_EXE_NAME} ${MPI_CXX_LIBRARIES})
    endif()

    if (MPI_CXX_INCLUDE_PATH)
      target_include_directories(${MYPROJ_EXE_NAME} PRIVATE ${MPI_CXX_INCLUDE_PATH})
    endif()
    if (MPI_CXX_COMPILE_FLAGS)
      target_compile_options(${MYPROJ_EXE_NAME} PRIVATE ${MPI_CXX_COMPILE_FLAGS})
    endif()

    if (MPI_CXX_LINK_FLAGS)
      set_target_properties(${MYPROJ_EXE_NAME} PROPERTIES
        LINK_FLAGS "${MPI_CXX_LINK_FLAGS}")
    endif()
  endif()
endfunction()
...
# Auxiliary function, used in myproj_find_package().
function(myproj_find_component Prefix DirVar IncSuffixes Header LibSuffixes Lib
         IncDoc LibDoc)
...
endfunction(myproj_find_component)


#   MYPROJ version of find_package that searches for header/library and if
...
#
function(myproj_find_package Name Prefix DirVar IncSuffixes Header LibSuffixes
         Lib IncDoc LibDoc)
  ...
  myproj_find_component("${Prefix}" "${DirVar}" "${IncSuffixes}" "${Header}"
  ...
  while(I LESS ARGC)
    ...
    elseif ("${ARGV${I}}" STREQUAL "ADD_COMPONENT")
      ...
      if (CompRequested)
        ...
        myproj_find_component("${FullPrefix}" "${DirVar}"
...
endfunction(myproj_find_package)
...
#   This function is intended to be called from the script Find${Name}.cmake
#
function(myproj_find_library Name Prefix Lib LibDoc CheckVar CheckSrc)
...
endfunction(myproj_find_library)
...
#   and 'test.mk'.
#
function(myproj_export_mk_files)
  ...
  set(CONFIG_MK_BOOL_VARS MYPROJ_USE_MPI MYPROJ_USE_METIS MYPROJ_USE_METIS_5
      MYPROJ_DEBUG MYPROJ_USE_EXCEPTIONS MYPROJ_USE_GZSTREAM MYPROJ_USE_LIBUNWIND
      MYPROJ_USE_LAPACK MYPROJ_THREAD_SAFE MYPROJ_USE_OPENMP MYPROJ_USE_MEMALLOC
      MYPROJ_USE_SUNDIALS MYPROJ_USE_MESQUITE MYPROJ_USE_SUITESPARSE MYPROJ_USE_SUPERLU
      MYPROJ_USE_STRUMPACK MYPROJ_USE_GECKO MYPROJ_USE_GNUTLS MYPROJ_USE_NETCDF
      MYPROJ_USE_PETSC MYPROJ_USE_MPFR MYPROJ_USE_SIDRE MYPROJ_USE_CONDUIT
      MYPROJ_USE_PUMI)
  ...
  endforeach()
  set(MYPROJ_CXX ${CMAKE_CXX_COMPILER})
  set(MYPROJ_CPPFLAGS "")
  string(STRIP "${CMAKE_CXX_FLAGS_${BUILD_TYPE}} ${CMAKE_CXX_FLAGS}"
         MYPROJ_CXXFLAGS)
  set(MYPROJ_TPLFLAGS "")
  foreach(dir ${MYPROJ_TPL_INCLUDE_DIRS})
    set(MYPROJ_TPLFLAGS "${MYPROJ_TPLFLAGS} -I${dir}")
  endforeach()
  # TODO: MYPROJ_TPLFLAGS: add other TPL flags, in addition to the -I flags.
  set(MYPROJ_INCFLAGS "-I\$(MYPROJ_INC_DIR) \$(MYPROJ_TPLFLAGS)")
  set(MYPROJ_PICFLAG "")
  if (BUILD_SHARED_LIBS)
    set(MYPROJ_PICFLAG "${CMAKE_SHARED_LIBRARY_CXX_FLAGS}")
  endif()
  set(MYPROJ_FLAGS "\$(MYPROJ_CPPFLAGS) \$(MYPROJ_CXXFLAGS) \$(MYPROJ_INCFLAGS)")
  # TPL link flags: set below
  set(MYPROJ_EXT_LIBS "")
  if (BUILD_SHARED_LIBS)
    set(MYPROJ_LIBS "${shared_link_flag}\$(MYPROJ_LIB_DIR) -L\$(MYPROJ_LIB_DIR)")
    set(MYPROJ_LIBS "${MYPROJ_LIBS} -lmyproj \$(MYPROJ_EXT_LIBS)")
    set(MYPROJ_LIB_FILE "\$(MYPROJ_LIB_DIR)/libmyproj.${myproj_VERSION}\
${CMAKE_SHARED_LIBRARY_SUFFIX}")
    set(MYPROJ_SHARED YES)
    set(MYPROJ_STATIC NO)
  else()
    set(MYPROJ_LIBS "-L\$(MYPROJ_LIB_DIR) -lmyproj \$(MYPROJ_EXT_LIBS)")
    set(MYPROJ_LIB_FILE "\$(MYPROJ_LIB_DIR)/libmyproj.a")
    set(MYPROJ_SHARED NO)
    set(MYPROJ_STATIC YES)
  endif()
  set(MYPROJ_BUILD_TAG "${CMAKE_SYSTEM}")
  set(MYPROJ_PREFIX "${CMAKE_INSTALL_PREFIX}")
  # For the next 4 variable, these are the values for the build-tree version of
  # 'config.mk'
  set(MYPROJ_INC_DIR "${PROJECT_BINARY_DIR}")
  set(MYPROJ_LIB_DIR "${PROJECT_BINARY_DIR}")
  set(MYPROJ_TEST_MK "${PROJECT_SOURCE_DIR}/config/test.mk")
  set(MYPROJ_CONFIG_EXTRA "MYPROJ_BUILD_DIR ?= ${PROJECT_BINARY_DIR}")
  set(MYPROJ_MPIEXEC ${MPIEXEC})
  if (NOT MYPROJ_MPIEXEC)
    set(MYPROJ_MPIEXEC "mpirun")
  endif()
  set(MYPROJ_MPIEXEC_NP ${MPIEXEC_NUMPROC_FLAG})
  if (NOT MYPROJ_MPIEXEC_NP)
    set(MYPROJ_MPIEXEC_NP "-np")
  endif()
  # MYPROJ_MPI_NP is already set
  # Define the variable 'MYPROJ_EXT_LIBS': handle PUMI libs
  if ("${MYPROJ_USE_PUMI}" STREQUAL "YES")
    ...
    foreach(pumilib ${pumi_dep_libs})
      ...
      if (NOT "${sharedlib}" MATCHES "-1" OR
        ...
        set(MYPROJ_EXT_LIBS "${pumilib} ${MYPROJ_EXT_LIBS}")
      elseif (NOT "${lib}" MATCHES "lib-NOTFOUND")
        set(MYPROJ_EXT_LIBS "${lib} ${MYPROJ_EXT_LIBS}")
  ...
  # Define the variable 'MYPROJ_EXT_LIBS': handle other (not PUMI) libs
  foreach(lib ${TPL_LIBRARIES})
    ...
    elseif ("${suffix}" STREQUAL "${CMAKE_SHARED_LIBRARY_SUFFIX}")
      ...
      set(MYPROJ_EXT_LIBS
          "${MYPROJ_EXT_LIBS} ${shared_link_flag}${dir} -L${dir} -l${libname}")
    else()
      set(MYPROJ_EXT_LIBS "${MYPROJ_EXT_LIBS} ${lib}")
    endif()
  endforeach()
  ...
  set(MYPROJ_INC_DIR "${CMAKE_INSTALL_PREFIX}/include")
  set(MYPROJ_LIB_DIR "${CMAKE_INSTALL_PREFIX}/lib")
  set(MYPROJ_TEST_MK "${CMAKE_INSTALL_PREFIX}/share/myproj/test.mk")
  set(MYPROJ_CONFIG_EXTRA "")
  ...
  install(FILES ${PROJECT_SOURCE_DIR}/config/test.mk
    DESTINATION ${CMAKE_INSTALL_PREFIX}/share/myproj/)
  install(FILES ${PROJECT_BINARY_DIR}/config/config-install.mk
    DESTINATION ${CMAKE_INSTALL_PREFIX}/share/myproj/ RENAME config.mk)
